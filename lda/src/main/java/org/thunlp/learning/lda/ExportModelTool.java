...
/**
 * Export the accumulated NWZs from last n iterations to an independent text file. Other software can then read the
 * model in plain text file, which will make the resulted model independent of Hadoop framework.
 * 
 * @author Xiance SI
 */
public class ExportModelTool implements GenericTool {
	...
	public void loadModel(Path model, int n) throws IOException {
		...
		ins.close();
		LOG.info("Load model parameters, alpha:" + alpha + " beta:" + beta + " num_topics:" + numTopics);
